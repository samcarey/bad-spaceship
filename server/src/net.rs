...
use bad_spaceship_shared::part::{
    local_contact_anchor, spawn_random_part, PartShape, SuppressLocalParts, DELETE_RADIUS, NUM_PARTS,
};
...
fn spawn_room_world(commands: &mut Commands, room: Room) {
    for _ in 0..NUM_PARTS {
        let (entity, shape, seed) = spawn_random_part(commands);
        tag_room_part(commands, entity, shape, seed, room);
    }
}
                    ...
                    for endpoint in [held_entity, other] {
                        if !had_joint.contains(&endpoint) && !replaced.contains(&endpoint) {
                            ...
                            if let Some(room) = held_room {
                                let (new_entity, shape, seed) = spawn_random_part(&mut commands);
                                tag_room_part(&mut commands, new_entity, shape, seed, room);
                            }
                        }
                    }
                }
