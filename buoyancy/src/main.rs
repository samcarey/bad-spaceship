...
/// Starting body density — below water, so it floats ~60% submerged.
...
/// Orbit-camera state: yaw/pitch around `target`, `distance` back from it.
/// Written by `orbit_input` (drag = orbit, scroll/pinch = zoom), applied to the
/// camera transform by `update_camera`.
#[derive(Resource)]
struct OrbitCamera {
    yaw: f32,
...
fn main() {
    App::new()
        ...
        // Shape rebuild → reset (repositions) → density → buoyancy (writes Forces).
...
fn setup(
    ...
    commands.spawn((
        ...
        MeshMaterial3d(materials.add(StandardMaterial {
            base_color: Color::srgba(0.1, 0.45, 0.85, 0.35),
    ...
    // The buoyant body. `ColliderDensity` lets Avian derive mass AND angular
    // inertia from the collider at the slider's density, keeping the two
    // consistent — which matters now that rotation is free (righting/capsizing
    // torques act against the real inertia tensor). The visuals are children:
    // the lateral wall in army green and the two end discs in a lighter shade.
    let shape = params.shape;
    let side_material = materials.add(StandardMaterial {
        base_color: Color::srgb(0.29, 0.33, 0.13), // army green
        perceptual_roughness: 0.6,
        ..default()
    });
    let cap_material = materials.add(StandardMaterial {
        base_color: Color::srgb(0.36, 0.40, 0.18), // slightly lighter army green
        perceptual_roughness: 0.6,
        ..default()
    });
    commands
        .spawn((
            ...
            ColliderDensity(params.density),
        ))
        .with_children(|body| {
            body.spawn((
                BuoyPart::Side,
...
/// Rebuild the render mesh + collider when a shape slider moves. The collider
/// swap makes Avian recompute mass properties at the current `ColliderDensity`.
fn rebuild_shape(
    params: Res<SimParams>,
    mut cache: Local<Option<Shape>>,
    mut meshes: ResMut<Assets<Mesh>>,
    mut bodies: Query<&mut Collider, With<Buoy>>,
    mut parts: Query<(&BuoyPart, &mut Mesh3d, &mut Transform)>,
) {
    let shape = params.shape;
    if *cache == Some(shape) {
        return;
    ...
    *cache = Some(shape);
    ...
    for (part, mut mesh, mut transform) in &mut parts {
        let old = mesh.0.clone();
        mesh.0 = match *part {
            BuoyPart::Side => meshes.add(frustum_side_mesh(shape)),
            BuoyPart::Cap { sign } => {
                *transform = cap_transform(shape, sign);
                meshes.add(cap_mesh(shape, sign))
            }
        };
        meshes.remove(&old);
    }
...
/// Push the slider's density onto the body (only when it changes). Avian reacts
/// to the `ColliderDensity` change by recomputing mass + angular inertia.
fn sync_density(params: Res<SimParams>, mut bodies: Query<&mut ColliderDensity, With<Buoy>>) {
    if !params.is_changed() {
        return;
    }
    for mut d in &mut bodies {
        d.0 = params.density;
    }
}
...
fn update_camera(
    ...
    transform.translation = orbit.target + rotation * (Vec3::Z * orbit.distance);
...
fn ui(
    ...
    egui::Area::new(egui::Id::new("source-link"))
    ...
    let panel = egui::TopBottomPanel::bottom("controls")
        ...
        .show(ctx, |ui| {
            ...
            labeled_slider(
                ui,
                "body density",
                egui::Slider::new(&mut params.density, 100.0..=2000.0).suffix(" kg/m³"),
            );
            labeled_slider(
                ui,
                "start angle",
